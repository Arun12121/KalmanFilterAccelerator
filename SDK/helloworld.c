...
int main()
{
	...
	*/
	...
	Xil_Out32 ((XPAR_SUM_ARR_0_S_AXI_CONTROL_BASEADDR+0x18), XPAR_BRAM_0_BASEADDR); //Sending image pointer address using offset

	Xil_Out32 ((XPAR_SUM_ARR_0_S_AXI_CONTROL_BASEADDR+0x00), 1);	//Start the IP

	while (0 == (2 & Xil_In32((XPAR_SUM_ARR_0_S_AXI_CONTROL_BASEADDR+0x00))));	//To check IP is done or not

	itof.a=Xil_In32 ((XPAR_SUM_ARR_0_S_AXI_CONTROL_BASEADDR + 0x10));	//Receiving returned value from HLS IP (converting because processor handles uint data) using address offset for returned argument
